	...
	case 'listar':
		$rspta=$encomiendas->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>($reg->condicion)?'<button class="btn btn-warning bg-green" onclick="mostrar('.$reg->guia.')"><i class="">Registrar</i></button>'.
 				
				'':
 				'<button class="btn btn-warning bg-green" onclick="mostrar('.$reg->guia.')"><i class="fa fa-check-square-o"> </i></button>'.
 				'',
                ...
                "7"=>$reg->fecha_re,
                 "8"=>$reg->precio.'<p> Bs.</p>',
					 ...
					 "9"=>($reg->condicion)?'<span class="label bg-blue">Almancen</span>':
